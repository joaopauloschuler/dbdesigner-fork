...
- Status: **FIXED** — the shim's `TSQLDataSet.InternalClose` (`src/clx_shims/sqlexpr.pas`) calls the new `TSQLConnection.ReleaseIdleTransaction`, which `CommitRetaining`s the transaction as soon as no dataset of the connection is open (COMMIT + deferred BEGIN, no lock); the shim `TClientDataSet` (`src/clx_shims/dbclient.pas`) closes a provider dataset it opened itself after the fetch, like Delphi's provider, so Query mode does not leave `OutputQry` open. `tests/TestSQLExprShim.pas` writes through a second connection while the first is idle (fails with "database is locked" on the old shim). Verified on the real display: app connected, `sqlite3 <db> "CREATE TABLE fix13_t(x)"` and INSERTs succeed, also right after a reverse engineering run; the new table shows up in the Reverse Engineering list.
...
- Status: **FIXED** — the MySQL path duplicated too (no name check anywhere), so both `EERMySQLReverseEngineer` and `EERSQLiteReverseEngineer` (`src/DBEERDM.pas`) now skip a table whose name is already in the model (`GetEERObjectByName(EERTable, ...)`), count it in the unit variable `RevEngSkippedTables`, append "N existing table(s) skipped." to the "Finished." status and the dialog shows an information box with the count (`src/EERReverseEngineering.pas`). Verified: `order.xml` + reverse engineering of the same DB (plus 3 new tables) -> "12 table(s) already exist in the model and were skipped.", saved model has 17 unique table names (`$S/fix13-skipmsg.png`, `$S/fix13-reveng.xml`).
...
- Status: **not a bug** — reproduced three times with xdotool (File menu click, Down x9, Return; the 9th item is "Close" whether or not connected, `$S/fix13-filemenu2.png` shows the highlight): the model closed each time (empty canvas) on `order.xml` after the reverse engineering, on a new "Noname2" model and on the saved reverse-engineered model (`$S/fix13-reveng-before.png` / `-after.png`). Note that the window title shows the *model name* stored in the XML (a model saved as `fix13-reveng.xml` is still titled "order") and is not cleared when the last model closes, so the title is no evidence either way; the original observation was most likely a lost `Return` on XWayland.
