...
| D | Compare reverse-engineered model with original | **works** (after #4, #5, #7; `$S/fix04-reveng.xml`, `$S/fix04-compare.txt`, `dbdtest` with all 11 FKs added by `$S/fix04-addfks.sql`) - 12 tables, all columns incl. AutoInc, `product_ean` UNIQUE, `product_name(name, info(100))`, all 11 relations with FK mapping and ON DELETE/UPDATE rules incl. the self-relation. Remaining differences: InnoDB's automatic `fk_*` FK indexes appear as extra INDEX entries; the self-relation is 1:n instead of 1:1; `Matching` lost. Before: (re-run after the fixes: `$S/fix01-reveng.xml`, `$S/fix01-compare.txt`, same result) — `$S/reveng.xml` vs `Examples/order.xml` with `compare_models.py` (`$S/compare.txt`): 12 tables, all columns by name/type/params/PK/NOT NULL/default; **AutoInc lost on all 5 auto-increment PKs** (#4); the UNIQUE index `product_ean` comes back as a plain INDEX (#5); 10 of 11 relations (self-relation `forumpost -> forumpost` not recoverable, as with SQLite), all with `CreateRefDef=1` / NO ACTION instead of the original's RESTRICT/CASCADE (the "Build Relations based on Primary Keys" heuristic never reads the FK metadata; #7) |
...
- **FIXED** (branch a3, see notes-to-myself.md "Fix: MySQL reverse engineering - AutoInc, UNIQUE/prefix indexes, native FK relations"). `EERMySQLReverseEngineer` reads `SHOW FIELDS` by name and sets `AutoInc` from `Extra` containing `auto_increment`; all 5 auto-increment PKs of `dbdtest` come back with `AutoInc="1"` (`$S/fix04-compare.txt`).
...
- **FIXED** (branch a3, same notes section). `SHOW KEYS` is read by name: `Non_unique=0` -> `ik_UNIQUE_INDEX`, `Sub_part` -> the index column's `LengthParam`; `product_ean` is UNIQUE again and `product_name` keeps `info(100)` (`$S/fix04-reveng.xml`). Note: InnoDB's automatically created FK indexes (`fk_<...>` on the FK column) are real indexes and are reverse engineered as plain INDEX entries the original model does not have.
...
- **FIXED** (branch a3, same notes section). Native relations come from `information_schema.KEY_COLUMN_USAGE` + `REFERENTIAL_CONSTRAINTS` (FK columns, `CreateRefDef=1`, ON DELETE/UPDATE codes, identifying when all FK columns are in the child's PK, self-references allowed), then the name/PK heuristic runs with `SkipExisting`. With all 11 FKs in `dbdtest` (9 added by hand, `$S/fix04-addfks.sql`) all 11 relations including `forumpost -> forumpost` and every rule are recovered. Not recoverable from MySQL: the 1:1 kind of the self-relation (comes back 1:n non-identifying) and `Matching` (MySQL ignores MATCH).
