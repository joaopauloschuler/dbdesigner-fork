...
| Stage | What | Status |
|---|---|---|
| A | Export `order.xml` to SQL create script, target "My SQL" (File > Export > SQL Create Script) | **works with one wrong result** — `$S/order_mysql.sql` (190 lines, inserts once, `AUTO_INCREMENT`, `info(100)` prefix, FKs) but the 3 InnoDB tables end in `TYPE=InnoDB` (#3) |
| B | `mysql dbdtest < script` | **3 errors** — `tests/mysql-roundtrip.sh $S/order_mysql.sql dbdtest` (`$S/roundtrip_A.txt`): `ERROR 1064 ... near 'TYPE=InnoDB'` for `webserver`, `webpageclick`, `weblog`; 9 of 12 tables created. With `TYPE=` replaced by `ENGINE=` (`$S/order_mysql_fixed.sql`, `$S/roundtrip_fixed.txt`): zero errors, 12 tables, both indexes (`product_ean` unique, `product_name(name, info(100))`), 2 FKs, 5 auto_increment columns, rows loaded once |
| C | New MySQL connection + connect + reverse engineer | **crash without the shim edits (#1, #2)**; with them **works** — the connection editor (driver MySQL, host, port 3306, database, user, password; `$S/conned2.png`) and connect work (server shows the session), the Reverse Engineering dialog lists the 12 tables (`$S/revdlg.png`) and Execute adds them with relations (`$S/main_rev.png`). The connection was not written to `DBConn.ini` (#6) |
| D | Compare reverse-engineered model with original | **mostly works** — `$S/reveng.xml` vs `Examples/order.xml` with `compare_models.py` (`$S/compare.txt`): 12 tables, all columns by name/type/params/PK/NOT NULL/default; **AutoInc lost on all 5 auto-increment PKs** (#4); the UNIQUE index `product_ean` comes back as a plain INDEX (#5); 10 of 11 relations (self-relation `forumpost -> forumpost` not recoverable, as with SQLite), all with `CreateRefDef=1` / NO ACTION instead of the original's RESTRICT/CASCADE (the "Build Relations based on Primary Keys" heuristic never reads the FK metadata; #7) |
...
| F | Query mode, `SELECT * FROM product` and an invalid statement | **works** (needs only #1) — grid with the 3 rows, status "Query opened. 3 Record(s) fetched" (`$S/query1_c.png`); `SELECT * FROM nosuch` -> "TMySQL80Connection : Error executing query: Table 'dbdtest.nosuch' doesn't exist" (`$S/query_err.png`) |
