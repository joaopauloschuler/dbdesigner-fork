...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, ComCtrls, FileUtil, Grids, ShellCtrls;
...
type
  ...
  TImageSelectionForm = class(TForm)
    ...
    DirectoryTreeView: TShellTreeView;
...
begin
  ...
  DirectoryTreeView.Path:=InitialDir;
...
begin
  ...
  {$IFDEF MSWINDOWS}
    ...
    searchMask := IncludeTrailingPathDelimiter(DirectoryTreeView.Path)+'*.*';
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          thePic.LoadFromFile(IncludeTrailingPathDelimiter(DirectoryTreeView.Path)+sr.name);



          ImgList.Add(thePic);

          ImageFileNameList.Add(IncludeTrailingPathDelimiter(DirectoryTreeView.Path)+sr.name);
